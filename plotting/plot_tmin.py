...
def main():
    ...
    dset = utils.read_dataset(variables=['air_temperature_min'], projection=projection)
    dset['air_temperature_min'] = dset['air_temperature_min'].metpy.convert_units('degC').metpy.dequantify()
...
def plot_files(dss, **args):
    ...
    for time_sel in dss.time:
        ...
        density = 18
        vals = utils.add_vals_on_map(args['ax'], args['m'],
                               data['air_temperature_min'],
                               args['levels_t2m'],
                               cmap=args['cmap'],
                               density=density)
        ...
        an_var = utils.annotation(args['ax'], 'Minimum 2m Temperature in previous 6 hours',
