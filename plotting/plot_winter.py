...
def main():
    ...
    dset = utils.read_dataset(variables=['integral_of_rainfall_amount_wrt_time',
                                         'liquid_water_content_of_surface_snow', 'altitude_of_0_degree_isotherm'],
                              projection=projection)

    rain = (dset['integral_of_rainfall_amount_wrt_time'] - dset['integral_of_rainfall_amount_wrt_time'][0, :, :])
    ...
    snow = (dset['liquid_water_content_of_surface_snow'] - dset['liquid_water_content_of_surface_snow'][0, :, :])
    snow = xr.DataArray(snow, name='snow_increment')

    dset = xr.merge([snow, rain, dset])
    ...
    dset = dset.drop(['liquid_water_content_of_surface_snow',
                      'integral_of_rainfall_amount_wrt_time']).load()
...
def plot_files(dss, **args):
    ...
    for time_sel in dss.time:
        ...
        c = args['ax'].contour(args['x'], args['y'], data['altitude_of_0_degree_isotherm'], levels=args['levels_snowlmt'],
